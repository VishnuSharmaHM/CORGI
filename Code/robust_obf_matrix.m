function [z, z_fval] = robust_obf_matrix(x_coord, y_coord, CPR_prior_prob, patient_index, z_last, NR_LOC, NR_TASK, DELTA, EPSILON, MIN_DISTANCE, PROB_LB)
...
%	A = zeros(NR_LOC*(NR_LOC-1)*NR_LOC, NR_LOC*NR_LOC);
    ...
    % A = sparse(NR_LOC*(NR_LOC-1)*NR_LOC, NR_LOC*NR_LOC); 
...
%     hold on; 
    ...
    for k = 1:1:NR_LOC       
        for i = 1:1:NR_LOC
            for j = 1:1:NR_LOC
%                 [i, inst(j), k]
                 distance = distance_matrix(i, j); % haversine([x_coord(1, i) y_coord(1, i)], [x_coord(1, j) y_coord(1, j)]); % sqrt((x_coord(1, i) - x_coord(1, j))^2 + (y_coord(1, i) - y_coord(1, j))^2); % distance between location i and location j
%                if ceil(i/7) == ceil(j/7) && ceil(k/7) == ceil(i/7)
                 if distance > 0
                    % max_r = maxratio(z_last, DELTA, NR_LOC, i, j); 
                    max_r = maxratioapprox(z_last, DELTA, NR_LOC, i, j, distance, EPSILON);
                    ...
                    max_r = maxratioapprox(z_last, DELTA, NR_LOC, j, i, distance, EPSILON);
                    max_r = min([max_r exp(EPSILON*distance)]); 
                    A(row_indx, (j-1)*NR_LOC+k) = max_r;
