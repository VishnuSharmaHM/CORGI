...
%                     [z_robust, z_fval_robust(epsilon, delta, times, iter)] = robust_obf_matrix(x_coord, y_coord, CPR_prior_prob, patient_index, z_robust_last, NR_LOC, NR_TASK, delta, epsilon_value(epsilon), MIN_DISTANCE, PROB_LB);
                   [z_robust, z_fval_robust(nr_loc_ratio, epsilon, delta, iter), nr_constraints(nr_loc_ratio)] = robust_obf_matrix2(x_coord, y_coord, CPR_prior_prob, patient_index, z_robust_last, NR_LOC, NR_TASK, delta, epsilon_value(epsilon), MIN_DISTANCE, PROB_LB);
...
%                     z_robust_diff(iter) = mean(mean(abs(z_robust - z_robust_last))); 
                ...
                running_time(nr_loc_ratio, epsilon, delta) = toc; 
                [z, z_fval(nr_loc_ratio, epsilon, delta)] = robust_obf_matrix2(x_coord, y_coord, CPR_prior_prob, patient_index, z, NR_LOC, NR_TASK, 0, epsilon_value(epsilon), MIN_DISTANCE, PROB_LB);
%                 [z, z_fval(epsilon, delta, times)] = obf_matrix(x_coord, y_coord, CPR_prior_prob, patient_index, NR_LOC, NR_TASK, epsilon, MIN_DISTANCE, PROB_LB); 
