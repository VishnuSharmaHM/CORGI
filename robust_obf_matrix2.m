function [z, z_fval] = robust_obf_matrix2(x_coord, y_coord, CPR_prior_prob, patient_index, z_last, NR_LOC, NR_TASK, DELTA, EPSILON, MIN_DISTANCE, PROB_LB)
...
%                     hold on; 
    ...
    for k = 1:1:NR_LOC
        ...
        for i = 1:1:NR_LOC
            inst = find(adjacency_matrix(i, :) > 0);
...
%	Aeq = zeros(NR_LOC, NR_LOC*NR_LOC);
    ...
    options=optimset('disp','iter','LargeScale','off','TolFun',.001,'MaxIter',100000,'MaxFunEvals',100000, 'Algorithm','dual-simplex');
